  ...
  const onSignInPressed = async () => {
    if (!value == "" && !password == "") {
      if (value.includes("@") == true && value.includes(".com") == true) {
        ...
        if (!res.ok) {
          if (!notify.isActive(id)) {
            return notify.show({
              id,
              title: "User with this email or password doesn't exist",
              avoidKeyboard: true,
              duration: 3000,
      ...
      if (!notify.isActive(id)) {
        return notify.show({
          id,
          title: "Please input your username/email and/or password!",
          avoidKeyboard: true,
          duration: 3000,
        });
      }
      ...
      <Box style={styles.container1}>
        ...
        <Box style={styles.container2}>
          <VStack alignItems={"center"} space={3}>
            ...
            <Button w="85%" onPress={onSignInPressed} bgColor="darkBlue.600">
              Sign In
            </Button>
