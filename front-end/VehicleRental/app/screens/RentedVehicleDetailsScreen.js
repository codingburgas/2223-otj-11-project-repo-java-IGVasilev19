...
const RentedVehicleDetailsScreen = () => {
  ...
  const onCancelRentPressed = async () => {
    const res = await fetch("http://192.168.1.4:8080/vehicle/cancel", {
      method: "POST",
      body: JSON.stringify({
        vin: vehicle.vin,
      }),
    ...
    if (res.ok) {
      ...
      if (!notify.isActive(id)) {
        return notify.show({
          id,
          title: "Rent canceled successfully!",
          avoidKeyboard: true,
          duration: 3000,
  ...
  return (
    <Box style={styles.root}>
      <VStack style={styles.container1}>
        <Text style={styles.text1}>Rented vehicle</Text>
...
export default RentedVehicleDetailsScreen;
